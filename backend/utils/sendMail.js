const sendMail = async (type, info) => {
    ...
    if(type === "contact") {
        url = script + `?toEmail=${toEmail}&subject=User want to contact from&body=<h5>Query travlio.in</h5><h4>Name : ${info.name}</h4><h4>Number : ${info.number}</h4><h4>Email : ${info.toEmail}</h4><h4>Message : ${info.message}</h4>`;
    }
