  ...
  return (
    <div className="w-full flex items-center relative inner">
      <Swiper
        ref={swiperRef}
        spaceBetween={50}
        slidesPerView={window?.innerWidth <= MOBILE_VIEW_WIDTH ? 1 : 3}
      ...
      >
        ...
        <SwiperSlide key={item.ID}>
          <li className={`toupPackagesShdow min-w-[365px] min-h-[365px] sm:min-w-full border border-[#e1e1e1] cursor-pointer ${currentIndex === index ? "scale-100" : "scale-75"} transition-all duration-1000`}>
      ...
      <button
        className={`absolute top-1/2 left-4 z-20 transform -translate-y-1/2 homeInfoSection bg-white bg-opacity-80 text-gray-800 p-3 rounded-full focus:outline-none ${currentIndex <= 0 ? "hidden" : "block"}`}
        onClick={onPrevBtnClicked}
      >
        <GrPrevious />
      </button>
      <button
        className={`absolute top-1/2 right-4 z-20 transform -translate-y-1/2 bg-white homeInfoSection bg-opacity-80 text-gray-800 p-3 rounded-full focus:outline-none ${currentIndex >= dataLength - 1 ? "hidden" : "block"}`}
